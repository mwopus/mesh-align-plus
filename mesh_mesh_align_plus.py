...
class ScaleMatchEdgeBase(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not (bpy.context.active_object and bpy.context.active_object.select_get()):
            self.report(
                {'ERROR'},
        ...
        if (bpy.context.active_object and
            ...
            multi_edit_targets = [
                ...
                # model for model in bpy.context.render_layer.objects if (
                    # model.select and model.type == 'MESH'
                    model.select_get() and model.type == 'MESH'
                )
            ]
...
class QuickAlignObjects(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # selected = [item for item in bpy.context.render_layer.objects if item.select]
